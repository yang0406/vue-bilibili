...
> 为什么是写哔哩哔哩(俗称B站)呢？其一是因为本人是B站的重度使用者，每周基本上都会在B站上看看火影忍者、暴走大事件等。另外一个原因呢，就是B站首页很美观大方，而且处处充满了细节。所以对于实践确实是一个不错的项目。目前的技术栈主要的采用的是前端Vue2+后端Koa2的前后端分离的方式，语法全部使用ES6。关于数据也来自真实的数据接口。

## 技术栈

前端：vue2 + vuex + webpack + ES6 + stylus
...
```shell
git clone https://github.com/yang0406/bilibili-vue.git
```
